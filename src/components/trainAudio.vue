...
<div style="height: 100vh; width: 100vw">
    <div class="train-panel">
        ...
        <button :class="['btn base-btn', isDone ? 'btn-red' : '']" :disabled="trainable || !wanted_word" @click="onTrain()">
    ...
    <b-modal id="inference" centered title="TEST" modal-class="my-modal-class" :hide-footer="true" @hidden="stopInference">
    ...
    <b-modal id="setting" centered title="MODEL CLASSIFICATION" modal-class="my-modal-class" :hide-footer="true">
        <label for="wanted-word">Wanted word:</label>
        <b-form-select id="wanted-word" :options="options" v-model="wanted_word"></b-form-select>
        <label for="learning-rate" class="mt-2">Learning rate:</label>
        <b-form-input id="learning-rate" type="number" min="0" max="1" step="0.0000001" v-model="learning_rate"></b-form-input>
        <label for="training-step" class="mt-2">Training step:</label>
        <b-form-input id="training-step" type="number" min="0" step="1" v-model="epochs"></b-form-input>
        <label for="threshold" class="mt-2">Training step:</label>
        <b-form-input id="threshold" type="number" min="0" max="1" step="0.01" v-model="categorical_threshold"></b-form-input>
    </b-modal>
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        onTrain: async function () {
            ...
            console.log(this.$store.getters.getAudiosClasses.length)
            console.log(initial_model_params[initial_model_params.length-1])
            var lastLayer = initial_model_params[initial_model_params.length-1]
            lastLayer.units = this.$store.getters.getAudiosClasses.length
            initial_model_params[initial_model_params.length-1] = lastLayer
            this.loading = true;
            this.isDone = false;
